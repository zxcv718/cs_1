...
프로젝트 디렉토리 구조는 평가자가 README만 읽고도 결과물 위치를 바로 따라갈 수 있도록 역할 기준으로 나눴습니다. `README.md`는 전체 설명과 검증 흐름을 담는 중심 문서이고, `Dockerfile`은 이미지 빌드 규칙, `site/`는 실제 웹 콘텐츠 원본, `docs/logs/`는 터미널 증거, `docs/screenshots/`는 브라우저와 VS Code 같은 화면 증거를 모아 두는 위치입니다. 즉 코드, 실행 규칙, 텍스트 증거, 화면 증거를 섞지 않고 분리해 두어 재현성과 검토 편의성을 높이려는 구조입니다.

이 구조를 이렇게 나눈 기준은 단순 폴더 분리가 아니라 책임과 변경 성격의 분리입니다. `Dockerfile`과 `site/`는 결과물을 만들어 내는 입력이고, `docs/logs/`와 `docs/screenshots/`는 실행 후 남는 검증 증거입니다. 둘을 같은 위치에 섞어 두면 "무엇이 실행 규칙인지"와 "무엇이 실행 결과인지"가 흐려지므로 분리했습니다.

또한 검토 동선을 짧게 만드는 것도 중요한 기준이었습니다. 평가자는 README 설명을 읽다가 바로 같은 저장소 안의 빌드 규칙, 웹 원본, 텍스트 로그, 화면 캡처로 이동할 수 있어야 합니다. 그래서 설명 중심 문서인 README를 루트에 두고, 실제 산출물과 증거를 역할별 하위 경로로 고정해 재현과 검토가 같은 흐름 안에서 이어지도록 설계했습니다.

마지막으로 재실행과 기록 보존이 서로 덜 간섭하도록 했습니다. 실습을 다시 하면 주로 바뀌는 것은 `Dockerfile`, `site/`, 일부 로그이지만, 이미 남겨 둔 스크린샷과 다른 종류의 증거까지 한꺼번에 흔들릴 필요는 없습니다. 그래서 입력 자산과 증거 자산을 분리해, 수정이 필요한 파일과 보존해야 하는 파일을 구조적으로 구분했습니다.
...
두 방식은 상황에 따라 선택 기준이 다릅니다. 상대 경로는 저장소 내부에서 이동성과 가독성이 좋아 README 링크, 소스 파일 참조, 같은 프로젝트 안에서의 명령 예시에 적합합니다. 다른 사람도 같은 저장소 루트에서 실행하면 그대로 이해할 수 있다는 점이 장점입니다.

반면 절대 경로는 현재 위치와 무관하게 특정 대상을 하나로 정확히 가리킬 수 있어서, 바인드 마운트처럼 Docker에게 "호스트의 실제 이 경로"를 넘겨야 할 때 더 안전합니다. 대신 절대 경로는 사용자 PC마다 달라 문서에 그대로 적어 두면 재현성이 떨어진다는 단점이 있습니다. 그래서 이번 README에서는 설명과 링크는 상대 경로를 주로 쓰고, 실제 바인드 마운트 명령에서는 `$(pwd)`를 사용해 각자의 절대 경로가 자동으로 들어가게 선택했습니다.
...
이 숫자 규칙이 이렇게 동작하는 이유는 리눅스가 권한을 문자 문장이 아니라 비트 플래그로 저장하기 때문입니다. 커널은 어떤 프로세스가 파일에 접근하려 할 때 먼저 "이 사용자가 소유자인가, 같은 그룹인가, 그 외 사용자인가"를 판별하고, 해당 범주에 연결된 `r`, `w`, `x` 비트가 켜져 있는지 검사합니다.

`r=4`, `w=2`, `x=1`이라는 값은 단순 암기용 숫자가 아니라 각 권한 비트의 자리값을 십진수로 표현한 것입니다. 그래서 `7`은 세 비트가 모두 켜진 상태이고, `5`는 읽기와 실행 비트만 켜진 상태를 압축해 적은 것입니다. 즉 `755` 같은 표기는 "소유자/그룹/기타 사용자"에 대한 세 묶음의 비트 상태를 짧게 적는 방법입니다.
...
이미지는 실행 설계도이고, 컨테이너는 그 이미지를 실제로 실행한 인스턴스입니다. 이 차이는 `빌드`, `실행`, `변경` 관점으로 보면 더 분명합니다.

- `빌드`: `docker build`는 Dockerfile과 소스 파일을 읽어 새로운 이미지를 만듭니다. 이 단계의 결과는 아직 실행되지 않은 템플릿입니다.
- `실행`: `docker run`은 그 템플릿을 바탕으로 실제 프로세스, 네트워크 설정, 쓰기 가능한 레이어를 가진 컨테이너를 만듭니다. 따라서 같은 이미지를 기반으로 이름과 포트만 바꿔 여러 컨테이너를 만들 수 있습니다.
- `변경`: 실행 중 컨테이너 안에서 파일을 바꾸면 우선 그 컨테이너의 쓰기 레이어만 바뀌고 원본 이미지는 그대로 남습니다. 앞으로 새로 만들 모든 컨테이너에 같은 상태를 넣고 싶다면 이미지를 다시 빌드해야 합니다. 반대로 바인드 마운트와 볼륨은 이미지 자체를 바꾸지 않고, 컨테이너 밖의 파일이나 데이터를 연결해 변화와 영속성을 다루는 방식입니다.
...
이유는 Docker 컨테이너가 기본적으로 호스트와 분리된 네트워크 네임스페이스를 사용하기 때문입니다. 컨테이너 안의 `localhost:80`은 컨테이너 자기 자신을 가리키고, 제 Mac에서 브라우저로 여는 `localhost:80`은 호스트 자기 자신을 가리킵니다. 둘 다 `localhost`라는 같은 단어를 쓰지만 실제로는 서로 다른 네트워크 공간입니다.

`-p 8080:80`을 주면 Docker가 호스트의 8080 포트와 컨테이너의 80 포트 사이에 전달 경로를 만듭니다. 이 격리 덕분에 여러 컨테이너가 내부에서는 모두 80번 포트를 사용해도 서로 충돌하지 않고, 실제 충돌은 호스트에 공개한 8080, 8081 같은 포트에서만 발생합니다. 참고로 `EXPOSE 80`은 "이 컨테이너가 이런 포트를 쓴다"는 문서화 성격의 힌트일 뿐, 외부 접속을 열어 주는 기능 자체는 `-p`가 담당합니다.
...
이번 재현에서는 관찰 대상을 섞지 않기 위해 포트도 의도적으로 분리했습니다. `8080`은 Dockerfile로 빌드한 이미지 결과를 확인하는 포트이고, `8081`은 호스트 파일을 실시간으로 읽는 바인드 마운트 전용 포트입니다. 이렇게 나누면 "이미지에 들어 있던 정적 결과"와 "호스트 변경이 즉시 반영된 결과"를 동시에 띄워 놓고 비교할 수 있습니다.
...
여기서는 익명 볼륨이 아니라 이름 있는 볼륨 `mydata`를 사용했습니다. 이름 있는 볼륨을 쓰면 첫 번째 컨테이너를 지운 뒤에도 두 번째 컨테이너가 같은 저장소를 명시적으로 다시 연결할 수 있어, "같은 데이터 공간을 다시 붙였다"는 사실을 재현 과정에서 더 분명하게 설명할 수 있습니다.
...
### 사례 4. 호스트 포트가 이미 사용 중일 때 포트 매핑이 실패하는 경우의 진단 순서

- 문제: `docker run -p 8080:80 ...` 실행 시, 호스트 포트가 이미 사용 중이면 컨테이너 실행이 실패할 수 있습니다.
- 왜 이상한가: 컨테이너 내부 80번 포트는 문제 없더라도, 호스트의 8080 포트가 다른 프로그램이나 다른 컨테이너에 이미 점유되어 있으면 Docker가 연결할 수 없습니다.
- 확인 순서:
  1. 먼저 `docker ps`로 이미 같은 포트를 쓰는 컨테이너가 있는지 확인합니다.
  2. Docker 컨테이너가 아니라 호스트 프로그램이 쓰고 있을 가능성도 있으므로 `lsof -i :8080` 같은 명령으로 포트 점유 프로세스를 확인합니다.
  3. 에러 메시지에 `address already in use` 같은 문구가 있는지 함께 봅니다.
- 해결: 기존 점유 프로세스나 컨테이너를 정리하거나, 필요하면 호스트 포트를 8081, 8082처럼 다른 번호로 바꿔 다시 실행합니다.
- 배운 점: 포트 매핑 실패는 컨테이너 자체 문제일 수도 있지만, 실제로는 호스트 포트 충돌인 경우가 많으므로 "컨테이너 상태 확인 → 호스트 포트 점유 확인 → 포트 변경 또는 정리" 순서로 보는 것이 효율적입니다.
...
이후 포트와 볼륨을 재현하는 절차는 임의로 명령을 나열한 것이 아니라, 비교 기준이 섞이지 않도록 설계했습니다. `8080`은 이미지 빌드 결과 확인 전용, `8081`은 바인드 마운트 비교 전용으로 나눠 같은 컨테이너 내부 80 포트를 쓰더라도 관찰 대상을 분리했습니다. 컨테이너 이름도 `codyssey-web-8080`, `bind-web`, `vol-test-evidence`, `vol-test2-evidence`처럼 역할이 드러나게 고정해 `docker ps`, `docker exec`, `docker rm` 결과를 읽기 쉽게 했습니다. 볼륨은 `mydata-evidence`처럼 이름 있는 볼륨을 사용해 첫 번째 컨테이너 삭제 후에도 두 번째 컨테이너가 정확히 같은 저장공간을 다시 붙이도록 설계했습니다. 또한 각 절차 앞에 정리 명령을 넣어, 평가자가 어느 시점에 다시 실행하더라도 같은 초기 상태에서 시작할 수 있게 했습니다.
